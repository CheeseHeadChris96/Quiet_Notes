(function(root){
  ...
  function validate(data){
    ...
    const folders=data.map(f=>{if(!f||typeof f.id!=='string'||!f.id||ids.has(f.id)||(f.parent!==null&&typeof f.parent!=='string'))throw new Error('Invalid folder.');const name=clean(f.name),pair=JSON.stringify([f.parent,key(name)]);if(siblings.has(pair))throw new Error('Duplicate folder name.');ids.add(f.id);siblings.add(pair);if(f.kind!==undefined&&!['gallery','gantt','todo'].includes(f.kind))throw new Error('Invalid folder type.');return {id:f.id,name,parent:f.parent,...(f.kind?{kind:f.kind}:{}),...(f.kind==='gantt'?{tasks:Gantt.validate(f.tasks||[])}:{}),...(f.kind==='todo'?{items:Todo.validate(f.items??[])}:{})};});
  ...
  const api={move,remove,ROOTS,LEGACY_ROOTS,clean,validate,path,children,create,load,merge};if(typeof module!=='undefined')module.exports=api;else root.Folders=api;
