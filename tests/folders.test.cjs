...
const roots=()=>F.LEGACY_ROOTS.map(f=>({...f}));
test('legacy customer and technology categories migrate to independent folders with one location',()=>{
 ...
 assert.equal(migrated.notes[2].folderId,null);
...
test('folder deletion removes its subtree and trashes only affected notes with valid recovery destinations',()=>{
 ...
 assert.deepEqual(result.notes[0],{...notes[0],folderId:null,deleted:true});assert.equal(result.notes[1].folderId,null);assert.deepEqual(result.notes[2],notes[2]);
 assert.equal(notes[0].deleted,false);assert.doesNotThrow(()=>F.load({version:5,folders:result.folders},result.notes));assert.doesNotThrow(()=>F.remove(folders,notes,'generic'));
...
test('folder moves preserve descendants and reject cycles, duplicate siblings, and invalid destinations',()=>{
 ...
 assert.throws(()=>F.move(folders,'hbs','hbs'));assert.throws(()=>F.move(folders,'hbs','net'));assert.throws(()=>F.move(folders,'hbs','missing'));assert.doesNotThrow(()=>F.move(folders,'generic','hbs'));
